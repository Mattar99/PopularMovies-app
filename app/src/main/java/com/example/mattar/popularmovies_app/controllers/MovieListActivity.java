...
import com.example.mattar.popularmovies_app.adapters.FavouriteMoviesAdapter;
...
import com.example.mattar.popularmovies_app.utils.EndlessRecyclerViewScrollListener;
...
public class MovieListActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private MoviesApiManagerSingleton.SortBy sortBy = MoviesApiManagerSingleton.SortBy.MostPopular;
    ...
    private MainResponse mMovies = new MainResponse();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (mRecyclerView.getAdapter() == null) {
                if (isNetworkAvailable()) {
                    ...
                    if (sortBy == MoviesApiManagerSingleton.SortBy.Favourite) {
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        MainResponse tempMovie = savedInstanceState.getParcelable(BUNDLE_MOVIES_KEY);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (sortBy == MoviesApiManagerSingleton.SortBy.TopRated)
            menu.findItem(R.id.menu_sort_top_rated).setChecked(true);
        else if (sortBy == MoviesApiManagerSingleton.SortBy.Favourite)
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            case R.id.menu_sort_popularity:
                if (sortBy != MoviesApiManagerSingleton.SortBy.MostPopular) {
                    if (isNetworkAvailable()) {
                        sortBy = MoviesApiManagerSingleton.SortBy.MostPopular;
                        loadMovies();
            ...
            case R.id.menu_sort_top_rated:
                if (sortBy != MoviesApiManagerSingleton.SortBy.TopRated) {
                    if (isNetworkAvailable()) {
                        sortBy = MoviesApiManagerSingleton.SortBy.TopRated;
                        loadMovies();
                        item.setChecked(true);
                        saveSortSelected();
                    } else {
            ...
            case R.id.menu_sort_favourite:
                if (sortBy != MoviesApiManagerSingleton.SortBy.Favourite) {
                    sortBy = MoviesApiManagerSingleton.SortBy.Favourite;
                    loadMovies();
                    item.setChecked(true);
                    saveSortSelected();
                }
    ...
    private void getMovies(final int page) {
        if (sortBy != MoviesApiManagerSingleton.SortBy.Favourite) {
            if (isNetworkAvailable()) {
                ...
                MoviesApiManagerSingleton.getInstance().getMovies(sortBy, page, new MoviesApiCallback<MainResponse>() {
                    @Override
    ...
    private void loadSortSelected() {
        sortBy = MoviesApiManagerSingleton.SortBy.fromId(getPreferences(Context.MODE_PRIVATE).getInt(getString(R.string.saved_sort_by_key), 0));
        setTitleAccordingSort();
    }
